...
async def scrape_cnas(attestation_number: str, employer_number: str):
    ...
    try:
        while attempts < max_attempts:
            attempts += 1
            logger.info(f"CNAS Verification Attempt {attempts} for {attestation_number}")
            
            try:
                url = f"{settings.CNAS_BASE_URL}{settings.CNAS_VERIFY_PATH}"
                await page.goto(url, timeout=settings.PAGE_LOAD_TIMEOUT_MS, wait_until="networkidle")
                
                captcha_img = await page.wait_for_selector('img[src*="simpleCaptcha.png"]', timeout=5000)
                if not captcha_img:
                ...
                await page.fill(settings.CNAS_FORM_ATTESTATION_SELECTOR, attestation_number)
                ...
                return {
                    **result,
                    "raw_response": html_content if settings.LOG_LEVEL == "debug" else None,
                    "attempts": attempts,
