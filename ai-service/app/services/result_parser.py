...
def parse_cnas_result(
    ...
    if (
        "Code de sécurité incorrect" in html_content
    ...
    if (
        "n'est pas valide" in html_content.lower()
        or "introuvable" in html_content.lower()
    ):
        result["valid"] = False
                    ...
                    if ssn_to_find in text:
                        ...
                        for i, cell in enumerate(cells):
                            if ssn_to_find in cell.get_text():
                                # Usually employee name is in the next cell
                                if len(cells) > i + 1:
